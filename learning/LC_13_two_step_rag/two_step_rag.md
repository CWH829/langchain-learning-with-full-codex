...
- [x] 能画出 indexing 与 query-time 两条流程。
- [x] 能解释为什么 2-step RAG 通常只有一次 chat model 调用。
- [x] 能独立完成 retrieve、format、generate 三段代码。
- [x] 能打印并核对真实检索来源。
- [x] 能测试知识库内问题与未知问题。
- [x] 能解释 top-k 不等于相关性保证。
- [x] 能说明 2-step RAG 相比 Agentic RAG 的优点与限制。
- [x] 能解释为什么检索上下文要使用分隔符并视为数据。

## 10. 实践记录

### 10.1 已知问题

- 问题 1：`How is short-term memory isolated?`
  - 检索来源：首先命中 `lc-10 / short-term-memory`，其次命中 `lc-11 / long-term-memory`。
  - 最终答案：根据 `lc-10` 的内容，回答使用 configurable `thread_id` 隔离不同 conversation，checkpointer 保存 conversation state。
  - 答案是否忠于来源：是。回答内容来自第一条检索文档，没有引入知识库外事实。

- 问题 2：`What does retrieval do before RAG generates an answer?`
  - 检索来源：依次命中 `lc-12 / retrieval` 与 `lc-13 / two-step-rag`。
  - 最终答案：检索先把文档转换为可搜索表示，并在查询时返回相关文档，然后 RAG 才生成答案。
  - 答案是否忠于来源：是。两条来源分别提供了 retrieval 的职责和 two-step RAG 的执行顺序。


两次已知问题中，`model.invoke(...)` 均返回 `AIMessage`。应用通过 `response.text` 取得最终文本，同时把真实检索到的 `list[Document]` 作为 sources 返回。

### 10.2 未知问题

- 问题：`What is the capital of France?`
  - 检索来源：空列表。
  - 模型是否明确回答不知道：是。模型说明给定上下文中没有相关文档，因此无法回答。
  - 结论：教学用 `KeywordEmbeddings` 对该问题生成全零查询向量；程序在调用 cosine similarity 前识别全零向量并返回空列表。`format_context(...)` 将空结果转换为 `No relevant documents were retrieved.`，system prompt 又要求资料不足时明确说不知道，因此 generation **没有使用模型自身知识猜测**。（因为关键是 system prompt 让了模型只根据检索上下文回答）


### 10.3 排错记录

- 现象：未知问题执行 `similarity_search(...)` 时抛出 `ValueError: NaN values found`。
- 原因：`KeywordEmbeddings` 只统计固定词表。未知问题没有命中任何关键词，查询向量为全零向量；余弦相似度的分母包含查询向量长度 0，最终形成 `0 / 0`，数值结果为 `NaN`。
- 解决方式：在检索前调用 `embed_query(question)`，通过 `if not any(query_vector)` 识别全零向量并直接返回空文档列表。相比捕获 LangChain 的异常消息，这种处理直接表达了教学 embedding 未命中词表的原因，也不会误吞其他 `ValueError`。
- 格式优化：为每个 Document 的 index、source、topic 和 content 增加换行，并使用 `---` 分隔文档；同时在 system instruction 与 `<context>` 之间增加换行，使上下文边界更清晰。

## 11. 阶段总结

本阶段完成了固定的 `retrieve -> format -> generate` 2-step RAG pipeline：

1. 原始 **question 直接进入向量检索**，**不经过 agent 决策**或 query rewrite。
2. 检索到的 `Document` 被格式化为带 source、topic 和分隔符的 context。
3. system prompt **约束模型只根据 context 回答**，并把检索内容视为数据。
4. 每个问题只调用一次 chat model，返回值确认是 `AIMessage`。
5. 应用**同时返回 answer 与真实 sources**，来源不依赖模型自行生成。

实践结果说明，已知问题可以命中对应知识并生成忠于来源的答案；未知问题通过全零向量检查转为空检索结果，模型按约束明确拒答。也进一步确认：2-step RAG 的可靠性由 retrieval 与 generation 共同决定，排错时应先检查真实 sources，再判断 prompt 和模型回答。

当前 `KeywordEmbeddings` 只是离线教学实现。它只能识别固定英文词表，不能代表真实 embedding 的语义能力；后续若替换真实 embedding，未知问题判断应更多依赖 similarity score、score threshold、reranking 或答案验证，而不是“是否为全零向量”。
