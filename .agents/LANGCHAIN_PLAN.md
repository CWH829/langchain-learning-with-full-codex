...
| 🟡 | LC-15 | MCP | MCP、`langchain-mcp-adapters`、docs MCP | 接入外部工具/文档 | 让 agent 调用 MCP server | async/await、客户端生命周期 | 2026-06-21 11:05 |  | R1,R2,R6 | 已创建本地 stdio 学习资料 server 与 LangChain MCP agent 手写骨架；本阶段先完成 tools、异步调用、消息流和默认 session 生命周期，resources/prompts 与远程 HTTP 作为边界了解。 |
